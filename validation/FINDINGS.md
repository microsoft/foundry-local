...
| F9 | python__ep-bootstrap__windows-x64__winml-dml | **major** | _TBD_ | open | _–_ | **windows-x64 accelerator divergence: RC exposes no DirectML/WinML EP.** `platform_manifest.json` lists windows-x64 accelerators as `[cpu, cuda, winml-dml, npu-winml, webgpu]` with **cuda + winml-dml marked *required***, but on this box `discover_eps()` returns only **CUDA / NvTensorRTRTX / OpenVINO / WebGpu** — there is **no DirectML or WinML EP** in the RC, and the `generic-gpu` variant runs `WebGpuExecutionProvider`, not DML. So `winml-dml` and `npu-winml` are unmet accelerator claims for windows-x64. **Real CUDA and real WebGPU inference both PASS** (qwen2.5-0.5b → 'Paris'), so the GPU-acceleration *capability* is satisfied by CUDA+WebGPU; the divergence is specifically the **DirectML/WinML + NPU claims**. **Action:** either (a) correct `platform_manifest.json` to drop winml-dml/npu-winml from windows-x64 "required" (make WebGPU/CUDA the required GPU path), or (b) treat as an RC gap if DirectML/WinML EP was intended to ship. Recorded as a **fail** (missing-but-expected variant, per ACCEPTANCE_POLICY) pending an owner decision. |
...
| F6 (win) | cs__chat__windows-x64__cpu | **major** | _TBD_ | fixed in branch `baijumeswani/f6-cs-shutdown-crash` (Linux); **re-verify on Windows** | _–_ | **F6 reproduced on windows-x64.** All five C# run cells (chat/embeddings/audio-file/web-server/model-mgmt) complete inference correctly (every functional marker + sanity assertion passes; the sample auto-selects the `cuda-gpu` variant on this GPU box and unloads cleanly) but the process then **exits `3221226324` = `0xC0000374` = STATUS_HEAP_CORRUPTION** at teardown — the Windows manifestation of the Linux exit-139 SIGSEGV in the same 1DS telemetry / `Manager` static-destruction path. Same root cause and same fix as F6 (subscribe to `AppDomain.ProcessExit` → Dispose while runtime healthy); the fix branch predates this run so the shipped RC still crashes here. Blocking on all C# cells (exit-code contract). |
...
- **Accelerator set:** `discover_eps()` returns **CUDA / NvTensorRTRTX / OpenVINO / WebGpu** — **no
  DirectML/WinML EP** and no NPU EP, though `platform_manifest.json` lists `winml-dml` + `npu-winml`
  as required for windows-x64 (**F9**, new). GPU acceleration *is* satisfied: **real CUDA** (`cuda-gpu`
  variant) **and real WebGPU** (`generic-gpu` variant) chat both return the exact expected token
  ('Paris'). So windows exposes **CPU + CUDA + WebGPU** (+ OpenVINO/NvTRTRTX registrable), unlike
  macOS (CPU+WebGPU) and linux-x64 (CPU+CUDA, no WebGPU) — windows-x64 is the only platform where
  **both** CUDA and WebGPU inference are verified.
...
- **New windows-only findings:** **F8** (Python wheel ships a **111 MB `foundry_local.pdb`** with 561
  ...
  (no DirectML/WinML/NPU EP vs manifest), **F10** (C# tool-calling/vision sample won't compile —
  `ToolChoice` ambiguous between the RC's new type and the Betalgo OpenAI lib).
...
- **What blocks GA (un-waived, blocking cells):**
  ...
  - **F6 (win)** — every **C# process crashes at teardown with `0xC0000374` (heap corruption)** after
    ...
    F6 SIGSEGV; the fix (`baijumeswani/f6-cs-shutdown-crash`, ProcessExit→Dispose) is not in the
    shipped RC.
  - **F1** — JS package ships **no license** (legal blocker; blocking `js__pkg-inspect` cell) —
    reproduced on Windows.
  - **F9** — RC exposes **no DirectML/WinML EP** (and no NPU EP) although `platform_manifest` marks
    `winml-dml` *required* for windows-x64 → the winml-dml cell is a missing-but-expected FAIL. Needs
    an owner decision: correct the manifest (make CUDA/WebGPU the required GPU path — both verified) or
    treat as an RC EP gap. GPU acceleration itself is **not** blocked (CUDA+WebGPU both work).
...
- **Bottom line:** the core **CPU / CUDA / WebGPU** runtime is **functionally sound** on windows-x64
  (real inference verified on all three paths, direct-API suites green, memory bounded). As-shipped
  the RC is **NO-GO** due to un-waived blocking-cell failures — **F8** (Python PDB packaging), **F6
  (win)** (C# heap-corruption at exit), **F1** (JS license), and the **F9** accelerator-manifest
  divergence. F6 already has a verified fix branch; F1/F8 are packaging fixes; F9 is a
  manifest-vs-RC reconciliation. Once those are resolved (or the affected SDK/accelerator claims are
  scoped out), the windows-x64 surface would be a **GO**.
