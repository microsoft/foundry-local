...
def rss_mb():
    # Cross-platform RSS in MB. Windows has no `ps`; use the Win32 API via ctypes there,
    # and fall back to `ps -o rss=` (KB) on macOS/Linux.
    if os.name == "nt":
        import ctypes
        ...
        # Declare signatures so the 64-bit pseudo-handle is not truncated to a 32-bit int.
