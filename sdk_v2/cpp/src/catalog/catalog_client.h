...
/// resolution and BYO synthesis. When `base_info` is provided, emits a CatalogFetch
/// event for the primary fetch and (if it runs) the cached-id lookup, copying the
/// endpoint/region/format/correlation fields from `base_info` and filling in
/// operation/status/duration/model_count/error.
