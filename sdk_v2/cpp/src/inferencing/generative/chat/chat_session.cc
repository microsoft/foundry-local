...
struct PreparedChatRequest {
  ...
  std::unique_ptr<chat_internal::PreparedChatMessages> projected_messages;
...
std::unique_ptr<ChatGenerator> CreateTextChatGenerator(
...
ToolCallContext BuildToolCallContext(const Request& request,
  ...
  chat_session_internal::PopulateToolDefinitions(tool_definitions, tool_ctx);
  ...
  for (const auto* item : request.items) {
    if (item->type == FOUNDRY_LOCAL_ITEM_TEXT) {
      ...
      if (text_item.text_type == FOUNDRY_LOCAL_TEXT_ITEM_TYPE_OPENAI_JSON) {
        ...
        auto extracted_definitions = request.prepared_tool_definitions.has_value()
        ...
        if (request.prepared_tool_definitions.has_value()) {
          ...
          request_tool_definitions = std::move(extracted_definitions);
        } else {
          request_tool_definitions = chat_session_internal::BuildJsonRequestToolDefinitions(
              std::move(extracted_definitions), tool_definitions);
        }
        ...
        prepared->projected_messages = std::make_unique<chat_internal::PreparedChatMessages>(
            message_preparer(prepared->all_messages, model.HasPositionalToolResults()));
        prepared->prompt = PrepareTextChatPrompt(*prepared->projected_messages, model, prepared->tool_context);
...
class ChatRequestPreflightOperation final : public Session::RequestPreflightOperation {
 public:
  ChatRequestPreflightOperation(ChatSessionStateSnapshot state, GenAIModelInstance& model,
                                ChatMessagePreparer message_preparer)
      : state_(std::move(state)), model_(model), message_preparer_(std::move(message_preparer)) {
...
std::unique_ptr<Session::RequestPreflightOperation> ChatSession::CaptureRequestPreflight(
  ...
  return std::make_unique<ChatRequestPreflightOperation>(std::move(state), model_, message_preparer_);
}
...
void ChatSession::ProcessRequestImpl(const Request& request, Response& response) {
  ...
  if (!cached_generator_) {
    ...
    if (media_turn) {
      ...
      generator = CreateTextChatGenerator(std::move(prepared.prompt), *prepared.projected_messages,
...
void ChatSession::ProcessChatCompletionsJson(PreparedChatRequest& prepared,
  ...
  // Create generator
  auto generator = CreateTextChatGenerator(std::move(prepared.prompt), *prepared.projected_messages,
