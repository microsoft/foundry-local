...
PreparedChatPrompt PrepareTextChatPrompt(const std::vector<TranscriptMessage>& messages,
  ...
  return PrepareTextChatPrompt(
      chat_internal::PrepareChatMessages(messages, model.HasPositionalToolResults()), model, tool_ctx);
}
...
PreparedChatPrompt PrepareTextChatPrompt(const chat_internal::PreparedChatMessages& messages,
  ...
  prepared.prompt =
      BuildChatPrompt(messages, model, tool_ctx.tools_json, tool_ctx.template_kwargs_json);
...
PreparedChatPrompt PrepareMediaChatPrompt(const std::vector<MessageItem>& messages,
  ...
  prepared.prompt = model.GetPreprocessor().ApplyChatTemplateWithOptions(
      messages_json.c_str(), tools, template_kwargs, true);
