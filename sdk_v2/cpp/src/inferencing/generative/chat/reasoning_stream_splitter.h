...
///
/// Marker token IDs are obtained by encoding the configured marker strings with the model tokenizer. Matching IDs
/// before inspecting decoded text is required for special tokens, whose decoded chunks can be empty when the
/// tokenizer skips special tokens. Token-prefix buffering also handles markers composed of multiple token IDs.
///
