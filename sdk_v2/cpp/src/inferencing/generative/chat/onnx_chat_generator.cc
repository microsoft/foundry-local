...
std::string OnnxChatGenerator::Decode() {
  ...
  // Diagnostic per-token trace (opt-in via FOUNDRY_LOCAL_DEBUG_TOKENS=1): records
  // the chosen token plus the top-2 logit margin. A near-zero margin flags a
  // greedy argmax that could flip under different FP rounding across hardware.
