...
// Reasoning-model chat templates (Qwen3, DeepSeek-R1, etc.) can pre-fill the
// reasoning open marker as part of the assistant generation prompt, e.g.
// `<|im_start|>assistant\n<think>\n`. When this happens the model's first
// generated byte is reasoning content — never the marker itself. The stream
// splitter must start in the reasoning state or it will forward chain-of-thought
// tokens to the caller as visible text and leak `</think>` when the model closes
// the block.
bool PromptOpensReasoning(const std::string& prompt, const std::string& reasoning_start) {
  if (reasoning_start.empty() || prompt.empty()) {
    return false;
  }
  const size_t end = prompt.find_last_not_of(" \t\r\n");
  if (end == std::string::npos) {
    return false;
  }
  const size_t trimmed_len = end + 1;
  if (trimmed_len < reasoning_start.size()) {
    return false;
  }
  return prompt.compare(trimmed_len - reasoning_start.size(), reasoning_start.size(), reasoning_start) == 0;
}
...
OnnxChatGenerator::OnnxChatGenerator(std::unique_ptr<OgaGeneratorParams> gen_params,
    ...
    : gen_params_(std::move(gen_params)),
      ...
      prompt_ends_in_reasoning_(prompt_ends_in_reasoning) {}
...
int OnnxChatGenerator::AppendMessages(const std::vector<MessageItem>& new_messages,
  ...
  // The assistant generation prompt is re-emitted for the new turn. Update the
  // reasoning-prefill hint so the next stream splitter starts in the right state.
  if (!reasoning_start.empty()) {
    prompt_ends_in_reasoning_ = PromptOpensReasoning(prompt, reasoning_start);
  }
...
std::unique_ptr<OnnxChatGenerator> OnnxChatGenerator::CreateImpl(const std::vector<MessageItem>& messages,
  ...
  // Apply auto-generated tool guidance for both auto and required tool choice. The grammar itself permits text when
  // text_output is true. ChatSession invalidates the generator after guided turns because a completed finite grammar
  // signals EOS and cannot be reused safely for continuous decoding.
  bool tool_guidance_enabled = tool_ctx.tool_output && tool_ctx.HasTools();
  ...
  // Detect whether the chat template's assistant generation prompt pre-fills
  // the reasoning open marker. If so, the model starts generating inside a
  // reasoning block; the stream splitter must be initialized accordingly.
  const std::string reasoning_start_marker =
      tool_ctx.reasoning_start.empty() ? std::string("<think>") : tool_ctx.reasoning_start;
  const bool prompt_ends_in_reasoning =
      tool_ctx.supports_reasoning && PromptOpensReasoning(prompt, reasoning_start_marker);
