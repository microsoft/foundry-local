...
TEST(CApiTest, SupportedVersionsReturnSameExpandedApiTables) {
  ...
  const flApi* v4 = FoundryLocalGetApi(4);
...
TEST(CApiTest, RequestPreflightVersionRangeAcceptsV3) {
  EXPECT_FALSE(fl::detail::IsRequestPreflightVersionSupported(2));
  EXPECT_TRUE(fl::detail::IsRequestPreflightVersionSupported(3));
  EXPECT_FALSE(fl::detail::IsRequestPreflightVersionSupported(4));
}
...
TEST(CApiTest, InferenceApiV3AppendsOnlyPreflightAtOffset22) {
  EXPECT_EQ(FOUNDRY_LOCAL_API_VERSION, 3);
  EXPECT_EQ(FOUNDRY_LOCAL_REQUEST_PREFLIGHT_MIN_VERSION, 3);
  ...
  const auto* api_v3 = FoundryLocalGetApi(3);
  ASSERT_NE(api_v3, nullptr);
  const auto* inference = api_v3->GetInferenceApi();
  ASSERT_NE(inference, nullptr);
  EXPECT_NE(inference->Session_PreflightRequest, nullptr);
}

TEST(CApiTest, RequestPreflightV3LayoutIsFrozenWithoutAvailableTokens) {
