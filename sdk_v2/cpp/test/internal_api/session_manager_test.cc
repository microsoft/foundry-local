...
TEST(SessionLockingTest, SessionOptionMutationDoesNotWaitForActiveRequestExecution) {
  ...
  auto mutation = std::async(std::launch::async, [&] {
    ...
    options.Add("max_output_tokens", "16");
    session.SetSessionOptions(options);
  });
  const auto mutation_status = mutation.wait_for(std::chrono::milliseconds(250));

  release_promise.set_value();
  EXPECT_EQ(execution.wait_for(std::chrono::seconds(1)), std::future_status::ready);
  execution.get();

  EXPECT_EQ(mutation_status, std::future_status::ready)
      << "SetSessionOptions waited for request_mutex_ and can deadlock a blocking streaming callback";
