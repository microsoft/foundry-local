...
TEST(ReasoningStreamSplitterTest, ToolCallShapedReasoningNeverReachesToolAccumulator) {
  ...
  const auto route = [&](const std::vector<Segment>& emitted) {
    ...
    for (const auto& segment : emitted) {
      ...
      calls.insert(calls.end(),
                   std::make_move_iterator(output.ready_calls.begin()),
                   std::make_move_iterator(output.ready_calls.end()));
    }
  };
  ...
  visible += final_output.visible_text;
  calls.insert(calls.end(),
               std::make_move_iterator(final_output.ready_calls.begin()),
               std::make_move_iterator(final_output.ready_calls.end()));
