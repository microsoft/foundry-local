...
struct ModelCtorToken {
  ...
  // Keeps the native manager alive even if JavaScript explicitly disposes its Manager wrapper.
...
class Model : public Napi::ObjectWrap<Model> {
 public:
  ...
  std::shared_ptr<foundry_local::Manager> manager_keepalive() const noexcept { return manager_keepalive_; }
