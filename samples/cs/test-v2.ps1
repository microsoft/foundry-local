<#
.SYNOPSIS
    Validate samples/cs against locally built or release-candidate packages.
...
.DESCRIPTION
    ...
    native binary and bundles WinML acceleration on Windows.
        ...
        Local mode (the default) wires the samples to a local C++ build:
    ...
    ReleaseCandidate mode skips C++/C# build and packing. It restores the
    requested Microsoft.AI.Foundry.Local version exactly from the public
    ORT-Nightly feed, with nuget.org available for transitive dependencies.

    No sample csproj files are modified at runtime. The package version is
    selected with an MSBuild command-line property.

.PARAMETER Mode
    Package source mode: Local or ReleaseCandidate. Default: Local.

.PARAMETER PackageVersion
    Exact Microsoft.AI.Foundry.Local version used in ReleaseCandidate mode.
    Default: 2.0.0-rc1.
...
.PARAMETER SkipBuild
    In Local mode, skip the C++ build step. Use when iterating after a fresh
    `python sdk_v2/cpp/build.py` invocation.

.PARAMETER SkipPack
    In Local mode, skip building the C++ SDK AND repacking the Runtime + SDK
    nupkgs. Reuses whatever this script packed previously. Implies -SkipBuild.
...
.EXAMPLE
    pwsh ./test-v2.ps1 -Mode ReleaseCandidate -Sample model-management-example
    # Restore Microsoft.AI.Foundry.Local 2.0.0-rc1 from ORT-Nightly and build one sample.

.EXAMPLE
    pwsh ./test-v2.ps1 -Mode ReleaseCandidate -PackageVersion 2.0.0-rc2
    # Restore and build every sample against exactly version 2.0.0-rc2.
#>
[CmdletBinding()]
param(
...
$isReleaseCandidate = $Mode -eq 'ReleaseCandidate'
...
$rcPackagesDir = Join-Path $cppDir 'build\samples-cs-rc-packages'
New-Item -ItemType Directory -Force -Path $configsDir | Out-Null
if (-not $isReleaseCandidate) {
    New-Item -ItemType Directory -Force -Path $feedDir | Out-Null
} else {
    New-Item -ItemType Directory -Force -Path $rcPackagesDir | Out-Null
}
...
if (-not $isReleaseCandidate) {
    ...
    Write-Host "Release candidate version: $PackageVersion" -ForegroundColor DarkGray
}
...
# ---------- 1. build C++ ----------
if (-not $isReleaseCandidate -and -not $SkipBuild) {
...
if (-not $isReleaseCandidate -and
    -not (Test-Path (Join-Path $cppBuildDir 'foundry_local.dll')) -and
...
if (-not $isReleaseCandidate -and -not $SkipPack) {
...
<configuration>
    ...
    <packageSourceMapping>
        ...
        <packageSource key="ORT-Nightly">
            ...
            <package pattern="Microsoft.ML.OnnxRuntimeGenAI.Foundry" />
        </packageSource>
    </packageSourceMapping>
</configuration>
...
$packageVersionArgs = @()
if ($isReleaseCandidate) {
    $packageVersionArgs += "/p:FoundryLocalPackageVersion=$PackageVersion"
}
$restoreSourceArgs = @()
if ($isReleaseCandidate) {
    $restoreSourceArgs += @('--packages', $rcPackagesDir, '--no-cache')
}
...
foreach ($s in $samples) {
    ...
    try {
        ...
        & dotnet restore $s.Csproj --configfile $sampleConfig --force @restoreSourceArgs @packageVersionArgs 2>&1 |
            Write-Host
        ...
        & dotnet build $s.Csproj --no-restore --configuration Debug @packageVersionArgs 2>&1 | Write-Host
        if ($LASTEXITCODE -ne 0) {
